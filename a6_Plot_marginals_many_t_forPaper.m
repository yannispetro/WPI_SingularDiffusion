...
for it = 1:length(tfs)
    ...
    if exist(WPI_Ritz_file, 'file') == 2 
        ...
        fprintf('==================================================================================== \n')
        for ik = iks(it):iks(it) % length(Mu)
            PDF = reshape(exp(-exponent(ik,:)), points(1),points(2),points(3),points(4));

            plot_iter_forPaper( x1, x2, x3, x4, PDF,'x' );
...
figure(1)
legend({'MCS - t = 1.0 s','WPI - t = 1.0 s','MCS - t = 3.0 s','WPI - t = 3.0 s'},'Interpreter','latex','FontSize',17)
set(gca,'fontsize',25)
xlim([x1(1),x1(end)])
